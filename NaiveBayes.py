...
class NaiveBayes:
    ...
    def __init__(self):
        ...
        self.pos_doc_count = 0
        self.neg_doc_count = 0
        self.total_doc_count = 0

        # Total words across all files
        self.total_words = 1

        # For Binary NaiveBaYes
        self.pos_docs_word_counts = collections.defaultdict(lambda: 0)
        self.neg_docs_word_counts = collections.defaultdict(lambda: 0)
    ...
    def classify(self, words):
        ...
        if(self.naiveBayesBool == False):

            pos_probability = math.log(self.pos_doc_count) - math.log(self.total_doc_count)
            neg_probability = math.log(self.neg_doc_count) - math.log(self.total_doc_count)

            for w in words:
                pos_probability += math.log(self.pos_words[w] + 1) - math.log(self.pos_words_count)
                neg_probability += math.log(self.neg_words[w] + 1) - math.log(self.neg_words_count)
...
##      Binary NaiveBayes
        if (self.naiveBayesBool == True and self.bestModel != True):
            ...
            vocab = len(self.pos_unique) + len(self.neg_unique)

            for w in words:
                if w in seen_words:
                    continue

                pos_probability += math.log(self.pos_docs_word_counts[w] + 3) - math.log(self.pos_words_count + (3 * vocab))
                neg_probability += math.log(self.neg_docs_word_counts[w] + 3) - math.log(self.neg_words_count + (3 * vocab))
                seen_words.add(w)
...
##      Mu BestModel
        if self.bestModel == True:
            ...
            for w in words:
                ...
                pos_probability += math.log(self.pos_docs_word_counts[w] + 200) - math.log(self.pos_words_count + ( 150* vocab))
                neg_probability += math.log(self.neg_docs_word_counts[w] + 1.5) - math.log(self.neg_words_count + (1.5 * vocab))
                seen_words.add(w)
    ...
    def addDocument(self, classifier, words):
        ...
        for w in words:
            if classifier == 'pos':
                ...
                self.pos_docs_word_counts[w] += 1   # num of pos docs with w
                pos_seen.add(w)
                ...
                self.total_words += 1
                self.neg_docs_word_counts[w] += 1   # num of neg docs with w
                neg_seen.add(w)

        if classifier == 'pos':
            self.pos_doc_count += 1
        else:
            self.neg_doc_count += 1
